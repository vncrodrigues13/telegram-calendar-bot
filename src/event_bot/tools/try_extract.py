"""CLI: test extraction with no Telegram and no Calendar.
    ...
    uv run python -m event_bot.tools.try_extract "sábado tem niver da Ana, 15h"
...
✏️ correction path, which is otherwise reachable only through Telegram:
    ...
    uv run python -m event_bot.tools.try_extract "niver da Ana dia 12/03" \\
...
from event_bot.extract import extract_event, now_local, reextract_with_correction
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="python -m event_bot.tools.try_extract",
        description=__doc__,
